...
let betitle =
  process.env.NODE_ENV == "development"
    ...
    : {
        do: "https://cyxbsmobile.redrock.team/wxapi/cyb-permissioncenter",
        re: "https://cyxbsmobile.redrock.team/wxapi/cyb-teamapply",
      };
...
export default {
  ...
  computed: {
    style: function () {
      return this.nowphone.length != 11 || this.nowChosen.length == 0
        ? "styleDis"
        : "stylebutton";
    },
  },
    ...
    getJSON(req).then(function (resolve) {
      ...
      data.forEach((item) => {
        _this.departList.push({
          id: item.id,
          name: item.name.match("—") ? item.name.split("—")[1] : item.name,
          ischoose: false,
        }); //组织id
      });
    });
